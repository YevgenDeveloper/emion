...
async function init() {
  ...
  v.command('list', 'list all running environments').action(
    async function(this: Vorpal.CommandInstance, args: any, callback: any) {
      this.log(v.chalk['green']('Here is the list of currently running envs:'))
  ...
  return new Promise((resolve,reject) => {
    ...
    if (env && env.command) {
      if(env.currentPid !== undefined) {
        logDataForEnv(env.id, color, `ENVIRONMENT IS ALREADY RUNNING WITH PID ${env.currentPid}`)
        resolve()
      } else  {
...
async function executeEnvironement(env: ExecutionEnvironement, color?: string) {
  return new Promise(async (resolve, reject) => {
    ...
    const cmd = env.arguments ?  ChildProcess.spawn(env.command!, env.arguments, {
      cwd: path.join(configuration.repoPath, env.repo!),
      ...
      if(env.readyWhen) {
        if(env.readyWhen.consoleMessageIsFound) {
          ...
          if(matchedValue !== null) {
            logDataForEnv(env.id, color!, `ENVIRONMENT READY ON PORT ${matchedValue.port}`)
            resolve()
          }
        }
        if(env.readyWhen.portIsUp) {
          ...
          if(status === 'closed') {
            logDataForEnv(env.id, color!, `ENVIRONMENT READY ON PORT ${env.readyWhen.portIsUp}`)
            resolve()
          }
        }
      }
